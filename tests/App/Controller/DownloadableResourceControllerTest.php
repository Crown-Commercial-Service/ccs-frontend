...
class DownloadableResourceControllerTest extends AbstractControllerTestCase
{
    ...
    private function createMockDownloadableResource(int $id = 42, string $slug = 'some-slug'): Page
    {
        $mockContent = CMSContentMockFactory::createMockContent([
            'campaign_code' => 'CAMP123',
            'description'   => 'A great downloadable resource',
        ]);

        $mockPage = $this->createMock(Page::class);
        $mockPage->method('getId')->willReturn($id);
        $mockPage->method('getUrlSlug')->willReturn($slug);
        $mockPage->method('getTitle')->willReturn('My Downloadable Resource');
    ...
    {
        $mockPage = $this->createMockDownloadableResource();

        $this->mockWordpressApi->method('getPageByUrl')
            ->with('/news/downloadable/some-slug')
            ->willReturn($mockPage);

        $this->client->request('GET', '/downloadable-resource/request/42/some-slug');

        $this->assertResponseIsSuccessful();
        $html = $this->client->getResponse()->getContent();
        $this->assertStringContainsString('My Downloadable Resource', $html);
    ...
    public function testRequestReturns404WhenResourceNotFound(): void
    {
        ...
        $this->client->request('GET', '/downloadable-resource/request/42/missing-slug');
    ...
    {
        $mockPage = $this->createMockDownloadableResource(42, 'some-slug');
        ...
        $this->client->request('POST', '/downloadable-resource/request/42/some-slug', [
            'name'     => 'Jane Doe',
        ...
        $this->assertResponseRedirects('http://localhost//downloadable-resource/confirmation/42/some-slug/?');
    }
    ...
    public function testRequestPostWithInvalidDataRendersErrors(): void
    {
        ...
        $this->mockFormController->method('sendToSalesforceForDownload')->willReturn([
            'nameErr' => ['errors' => ['Enter your name'], 'link' => '#name'],
        ]);
    ...
    public function testShowRendersConfirmationSuccessfully(): void
    {
        ...
        $this->client->request('GET', '/downloadable-resource/confirmation/42/some-slug');

        $this->assertResponseIsSuccessful();
        $html = $this->client->getResponse()->getContent();
        $this->assertStringContainsString('You can now download this resource', $html);
    }
    ...
    public function testShowReturns404WhenResourceNotFound(): void
    {
        ...
        $this->client->request('GET', '/downloadable-resource/confirmation/42/missing-slug');
